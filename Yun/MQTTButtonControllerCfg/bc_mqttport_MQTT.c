...
static char **MQTT_topics[2];
static int MQTT_topic_count = 2;
...
mosquitto_subscribe(MQTT_mosq, NULL, MQTT_topics[i], MQTT_topic_qos);
}
...
void MQTT_setup() {
	...
	char *will_topic = NULL;
        ...
        MQTT_topics[0] = "ThingML";
MQTT_topics[1] = "ThingML2";
...
void MQTT_forwardMessage(uint8_t * msg, int size, uint16_t topicID) {
    int n, m, i;
    int length = size;
    ...
    *q = '\0';
    ...
    if(topicID == 65535) {int j;
for(j = 0; j < MQTT_topic_count; j++) {
printf("[MQTT] publish:\"%s\" on topic: %s \n", p, MQTT_topics[j]);
mosquitto_publish(MQTT_mosq, &MQTT_mid_sent, MQTT_topics[j], (length * 3 + 1), p, MQTT_qos, MQTT_retain);
}
} else {
printf("[MQTT] publish:\"%s\" on topic: %s \n", p, MQTT_topics[topicID]);
mosquitto_publish(MQTT_mosq, &MQTT_mid_sent, MQTT_topics[topicID], (length * 3 + 1), p, MQTT_qos, MQTT_retain);
}
